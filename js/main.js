...
(function() {
  'use strict';
  var Arc, Factions, NebeneinkunftMinAmounts, Rep, RepInspector, T, Viewport, formatCurrency, nebeneinkuenfteMinSum, repRadius, updateCheckboxLabelState;
  ...
  RepInspector = (function() {
    ...
    RepInspector.prototype.update = function(rep) {
      var item, row, table, tableBody, tableRow, _i, _len, _ref, _results;
      this.field('name').text(rep.name).attr('href', rep.url);
      this.field('faction').text(rep.fraktion);
      this.field('land').text(rep.land);
      this.field('mandate').text(T(rep.mandat));
      this.field('constituency').text(rep.wahlkreis);
      this.field('minSum').text(formatCurrency(rep.nebeneinkuenfteMinSum));
